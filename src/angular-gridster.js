	...
	return angular.module('gridster', [])
		...
		.controller('GridsterCtrl', ['gridsterConfig', '$timeout',
			function(gridsterConfig, $timeout) {
				 ...
				 * @param {Array} ignoreItems
				 */
				this.moveItemsLeftOrRight = function(items, col, row, ignoreItems) {
					if (!items || items.length === 0) {
						return;
					}
					items.sort(function(a, b) {
					...
					for (i = 0, l = items.length; i < l; ++i) {
						item = items[i];
						var hole = this.findHole();
						if (hole[0] > row || (hole[0] === row && hole[1] > col)) {
							this.moveItemRight(item, ignoreItems);
						} else {
							this.moveItemLeft(item, ignoreItems);
						}
						ignoreItems.push(item);
					}
				};
				...
				/**
				 * Moves an item left to a specified col
				 *
				 * @param {Object} item The item to move
				 * @param {Number} newRow The target row
				 * @param {Array} ignoreItems
				 */
				this.moveItemLeft = function(item, ignoreItems) {
					if (item.col === 0) {
				 ...
				 * @param {Array} ignoreItems
				 */
				this.moveItemRight = function(item, ignoreItems) {
					if (item.col === this.columns - 1) {
						item.col = 0;
						item.row++;
					} else {
						item.col++;
			...
			function($document, $window, GridsterTouch) {
				function GridsterDraggable($el, scope, gridster, item, itemOptions) {
					...
					function mouseDown(e) {
						...
						if ($el[0].classList.contains('gridster-no-drag')) {
							return false;
						}
